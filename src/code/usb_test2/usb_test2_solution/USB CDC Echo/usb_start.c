...
static uint32_t temp_buffer[64];
...
{
	cdcdf_acm_write((uint8_t *)usbd_cdc_buffer, count);//-2 here works to mangle message
...
{
	/* Echo data. */
	cdcdf_acm_read((uint8_t *)usbd_cdc_buffer, sizeof(usbd_cdc_buffer)); //-1 --> de-syncs readback buffer
...
static bool usb_device_cb_state_c(usb_cdc_control_signal_t state)
{
	if (state.rs232.DTR) {
		...
		cdcdf_acm_register_callback(CDCDF_ACM_CB_READ, (FUNC_PTR)usb_device_cb_bulk_out); //zzstophere this is the read of the callback function
		cdcdf_acm_register_callback(CDCDF_ACM_CB_WRITE, (FUNC_PTR)usb_device_cb_bulk_in); //in = write?
		/* Start Rx */
		cdcdf_acm_read((uint8_t *)usbd_cdc_buffer, sizeof(usbd_cdc_buffer));//breaks terminal if removed
	}
...
void cdcd_acm_example(void)
{
	...
	while (1) {//zzstophere loop here is active
